...
! do the main calculation to find hsal on the grid
	if (P%save_sht==0) then
		call calc_hsal('0', nph_sht, nth_sht, h_sht, rhoe, rhoo, ntrunc, hsal_sht, (P%messages >= 2))
	else
		call calc_hsal(dir_grid // 'temp/', nph_sht, nth_sht, h_sht, rhoe, rhoo, ntrunc, hsal_sht, (P%messages >= 2))
	endif
...
!***************************************************
if (P%messages >= 2) then
	write(*, '(a)', advance='no') '   e) Calculate beta (averaged over '
	call disp(tostring(P%sal_avg)//'º)... matgen', advance='no')
endif
...
!***************************************************
if (P%messages >= 2) then
	write(*, '(a)', advance='no') '   e) coo_mat_mul '
