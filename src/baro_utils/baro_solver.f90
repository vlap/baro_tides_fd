...
!%   ta_h : tau on the h-grid points
     ...
     type (csr_cmplx) :: speye
     integer, allocatable :: bcdiag(:), bcdiag_base(:)
...
!%==================================================
! Load the base matrix once to avoid redundant I/O
!%==================================================
     write(*, '("Loading initial matrix... ")', advance='no')
     call load_alloc_sparse(mat_base, dir_mats // 'temp/mat_init.dat')
     call load_alloc_vector(bcdiag_base, dir_mats // 'temp/bcdiag.dat')
     write(*, '("done.")')
