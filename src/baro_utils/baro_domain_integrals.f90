...
subroutine calc_convergence(nu, nv, nh, itn, P, cpts, delta, dir_cols, dir_sols)
	...
	ds = disp_get()
	  ...
	  CALL DISP('duv%', delta(itn,8,:), FMT='F6.3',TRIM='yes', advance='no')
	  ...
	  CALL DISP('dh%', delta(itn,7,:), FMT='F6.3',TRIM='yes', advance='no')
...
!**********************************************************************************************************
subroutine show_domain_integrals(cpt, di)
...
implicit none
     ...
     type(disp_settings) ds
		...
		call disp(char(9), '---------------------------------------------------------------------')
		ds = disp_get()
		  CALL DISP([char(9),char(9),char(9),char(9),char(9)], advance='no')
		  CALL DISP_SET(STYLE='underline', SEP=char(9), MATSEP=' | ')

		  CALL DISP('****', ['D_BL','D_IT', 'D   '], advance='no')
		  CALL DISP('Total', [di%dbl%total/1d12,di%dit%total/1d12,(di%dbl%total + di%dit%total)/1d12], FMT='F5.3',&
		  																					TRIM='yes', advance='no')
		  CALL DISP('Shallow', [di%dbl%shallow/1d12,di%dit%shallow/1d12,(di%dbl%shallow + di%dit%shallow)/1d12], &
		  																		FMT='F5.3', TRIM='yes', advance='no')
		  CALL DISP('Deep', [di%dbl%deep/1d12,di%dit%deep/1d12,(di%dbl%deep + di%dit%deep)/1d12], &
		  																		FMT='F5.3', TRIM='yes', advance='no')
		  CALL DISP('Coastal', [di%dbl%coastal/1d12,di%dit%coastal/1d12,(di%dbl%coastal + di%dit%coastal)/1d12], &
		  																		FMT='F5.3', TRIM='yes', advance='no')
		  CALL DISP('Open', [di%dbl%open/1d12,di%dit%open/1d12,(di%dbl%open + di%dit%open)/1d12], FMT='F5.3', &
		  																					TRIM='yes', advance='no')
		  CALL DISP(char(9))
...
subroutine baro_domain_integrals(cpt, P, nu, nv, nh, dir_grid, dir_cols, dir_mats, dir_sols, di)
...
!  %====================================================
    if (itd_scheme == 0) then
		Du = 0.
		Dv = 0.
	elseif (itd_scheme == 1) then
