...
! This function was used to perform some simple tests
     ...
     real(dp), allocatable	:: H_sht(:,:), topo_sht(:,:),  H_sht_copy(:,:)
...
!	integer :: m,n
	...
	logical :: which_test(4) = (/.false.,.false.,.true.,.false./)
...
if (which_test(1)) then
	...
	! Truncate a half of the spherical harmonics
	H_sht = H_sht_copy
	call test_shrotate1(H_sht, angles, (P%messages > 0), trim(dirname)//'sht_coeff_test.dat') ! spec_coeff_file
	call save_topo(trim(dirname)//'rotated_test1.dat', nlon_sht, nlat_sht, lon_sht, lat_sht, H_sht)
endif

if (which_test(2)) then
	! Sinc filter. ! Keep all of the spherical harmonics
	H_sht = H_sht_copy
	...
	! Sinc filter. ! Truncate a half of the spherical harmonics
	H_sht = H_sht_copy
	call test_shrotate21(H_sht, angles, (P%messages > 0), trim(dirname)//'sht_coeff_test.dat') ! spec_coeff_file
	call save_topo(trim(dirname)//'rotated_test21.dat', nlon_sht, nlat_sht, lon_sht, lat_sht, H_sht)

	! Sinc^2 filter. ! Keep all of the spherical harmonics
	H_sht = H_sht_copy
	call test_shrotate22(H_sht, angles, (P%messages > 0), trim(dirname)//'sht_coeff_test.dat') ! spec_coeff_file
	call save_topo(trim(dirname)//'rotated_test22.dat', nlon_sht, nlat_sht, lon_sht, lat_sht, H_sht)
	! Sinc^2 filter. ! Truncate a half of the spherical harmonics
	H_sht = H_sht_copy
	call test_shrotate221(H_sht, angles, (P%messages > 0), trim(dirname)//'sht_coeff_test.dat') ! spec_coeff_file
	call save_topo(trim(dirname)//'rotated_test221.dat', nlon_sht, nlat_sht, lon_sht, lat_sht, H_sht)
...
!**********************************************************************************************************
!**********************************************************************************************************
      subroutine test_shrotate1(datagrid, angles, messages, spec_coeff_file)!, datagrid_out)
...
!      real, dimension(size(datagrid,2),size(datagrid,2)) :: a, b
      ...
      integer ntrunc, n, m, nn, j, istat, k, deln
      logical :: messages
	 !real(wp), allocatable	:: ph_sht(:), th_sht(:)
     real    ::      T1, T2
	...
	real*8, intent(in)	:: angles(3)
	real*8, allocatable ::	cilm1(:,:,:), cilm2(:,:,:), smooth(:)
!	real*8  :: dj(size(datagrid,2),size(datagrid,2),size(datagrid,2))
...
!	Smoothing as a function of total wavenumber
	allocate(smooth(ntrunc+1), stat = istat)
	smooth = 0.
	deln = ntrunc + 1
!	Test sinc filter in the spectral space to mitigate Gibbs phenomenon
	do j = 1, ntrunc+1
		smooth(j) = sinc(float(j-1)/deln)!Gaussian spectral smoothing, where deln is the e-folding width of the smoother in total wavenumber space.
	enddo

	! Now apply to the coefficients
      cilm1 = 0.
      do m=1,ntrunc+1
      do n=m,ntrunc+1
         nn = sum((/(k,k=ntrunc+1,ntrunc-m+3,-1)/))+n-m+1
         cilm1(1,n,m) = 2.*real(dataspec(nn), kind=wp) * smooth(m) * smooth(n)
         cilm1(2,n,m) = 2.*imag(dataspec(nn)) * smooth(m) * smooth(n)
      enddo
      enddo
...
!**********************************************************************************************************
subroutine test_shrotate21(datagrid, angles, messages, spec_coeff_file)!, datagrid_out)
...
end subroutine test_shrotate21
!**********************************************************************************************************
subroutine test_shrotate22(datagrid, angles, messages, spec_coeff_file)!, datagrid_out)
...
!	Test sinc filter in the spectral space to mitigate Gibbs phenomenon
	...
	! Now apply to the coefficients
      ...
      do n=m,ntrunc+1
         nn = sum((/(k,k=ntrunc+1,ntrunc-m+3,-1)/))+n-m+1
         cilm1(1,n,m) = 2.*real(dataspec(nn), kind=wp) * (smooth(m) * smooth(n))**2
         cilm1(2,n,m) = 2.*imag(dataspec(nn)) * (smooth(m) * smooth(n))**2
      enddo
      enddo
...
end subroutine test_shrotate22
!**********************************************************************************************************
subroutine test_shrotate221(datagrid, angles, messages, spec_coeff_file)!, datagrid_out)
...
end subroutine test_shrotate221
!**********************************************************************************************************
...
!      real(wp), dimension(:,:), intent(inout), optional :: datagrid_out
      complex(dp), allocatable :: dataspec(:), smooth(:)
