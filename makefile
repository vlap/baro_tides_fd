...
ifeq ($(compiler),gnu) #******************* GFORTRAN ************************
  FC = gfortran
  OPTS = -fimplicit-none -ffree-form -ffixed-line-length-none -mcmodel=medium #-fcray-pointer  -fno-second-underscore #-x f77-cpp-input
  #FPPSTOP=-x none
...
ifeq ($(debug),work)
  OPTS += -g -w -fbounds-check -fbacktrace -finit-real=nan #-fconvert=big-endian !  -Wall (includes -Wunused -Wuninitialized) -Wextra
else
...
#Include directory with mod files
  ...
  OPTS = -w -132 -mcmodel=medium -i-dynamic#-fpp
...
MYSPARSE_L_S= my_blas.f90 my_sparse.f90 my_sparse_aggregate.f90
...
MODULES_P_S=precisions.f90 iso_varying_string.f90 
...
obj/baro_v1.o: baro_v1.f90 $(MODULES_H_O) $(MODULES_M_O) $(MODULES_L_O) $(MODULES_P_O) $(FGMRES_MKL_O)
	$(FC) -c ${OPTS} $< -o $@
